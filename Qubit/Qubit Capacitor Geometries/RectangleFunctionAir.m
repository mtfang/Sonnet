function [cap_pF] = RectangleFunctionAir(capSize, gapSize, boxSize, airThickness)
...
Project.replaceDielectricLayer(1,'Air', airThickness, 1, 1, 0, 0, 0);
...
c1 = [boxSize(1), boxSize(2)/2 - 5];
c2 = [boxSize(1) - boxGap, boxSize(2)/2 + 5];
...
[mb, sub] = Project.estimateMemoryUsage();
disp(['Memory (MB): ', num2str(mb)])
disp(['Subsections: ', num2str(sub)])
Project.addFileOutput('TS','D','Y',filenameTS,'IC','Y','Y','RI','R',50); 
Project.save();
Project.simulate('-c');
% Project.simulate('-c -t'); % Cannot hide status window if remote computing
